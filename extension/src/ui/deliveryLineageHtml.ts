...
<head>
  ...
  <style nonce="${nonce}">
    ...
    * { box-sizing: border-box; }
    ...
    main { display: grid; grid-template-columns: minmax(0, 1fr) 360px; height: calc(100vh - 104px); }
...
<body>
  ...
  <script nonce="${nonce}">
    ...
    function selectNode(id) {
      ...
      document.querySelectorAll('.node').forEach(function(card) { const isSelected = card.dataset.nodeId === id; const isConnected = !isSelected && connected.has(card.dataset.nodeId); card.classList.toggle('selected', isSelected); card.classList.toggle('connected', isConnected); card.classList.toggle('unrelated', !isSelected && !isConnected); });
