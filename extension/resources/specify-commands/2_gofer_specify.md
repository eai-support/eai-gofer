...
Record material decisions and update the goal ledger when new evidence changes
the delivery path. Do not ask the user to approve routine planning or design
choices. Pause only when an approval boundary in the Continuation And Stop
Contract applies. Otherwise read and follow
`.specify/commands/3_gofer_plan.md` in the same conversation. Do not ask for a
numbered command.
