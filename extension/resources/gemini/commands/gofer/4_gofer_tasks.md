...
- Storage binding validation -> after app provisioning, confirm `.eai/storage-bindings.json` was written, then run `eai types validate --tenant-key <key> --tenant-id <tenant-id>` and confirm app-owned aliases/prefixes before publish. Agents must not invent raw storage aliases or table names.
- Object-type publish -> `eai types seed` only after provisioning, storage binding validation, and local validation are complete.
