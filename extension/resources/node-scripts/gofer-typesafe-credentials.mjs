...
export async function requestTypeSafeEvaluation({ workspace, env = process.env, fetchImpl = globalThis.fetch, policy, projection, rubric }) {
  const { apiKey, source: credentialSource } = await resolveApiKey({ workspace, env });
  ...
  try {
    ...
    return { status: 'received', networkCalled: true, credentialSource, payload: JSON.parse(raw) };
  } catch {
