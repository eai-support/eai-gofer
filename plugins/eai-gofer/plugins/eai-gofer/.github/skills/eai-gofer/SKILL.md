...
Version: 3.7.22 Host: VS Code and GitHub Copilot

# EAI Gofer

Use this as the single user-facing Gofer command. Users should run `/eai-gofer`,
`$eai-gofer`, or `#eai-gofer` depending on the host. Do not ask users to run
numbered stage commands unless they explicitly request low-level internals.
...
4. If the user says yes, run
   `node .specify/scripts/node/gofer-workspace-bootstrap.mjs --host copilot --include-mirrors`,
...
3. If EAI CLI, login, tenant, or template readiness is missing, run the
   first-run/setup path from `.specify/commands/gofer_eai_first_run.md` when
   present.
