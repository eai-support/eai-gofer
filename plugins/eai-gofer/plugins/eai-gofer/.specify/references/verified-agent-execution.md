...
EAI Setup checks the machine prerequisites. EAI CLI reports the local
isolation contract for the selected surface. Gofer validates that report before
it creates the task worktree. The report is a readiness check, not proof that a
host executed the task. A fresh host receipt is still required.
