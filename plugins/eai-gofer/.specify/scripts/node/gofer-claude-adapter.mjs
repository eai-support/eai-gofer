...
/** The shared Git store of a linked worktree, or null. Like the Codex policy,
 * a plain repository is refused: its Git store lies inside the workspace. */
function sharedGitDirectory(workspaceRoot) {
  ...
  try {
    ...
    if (realpathSync(topLevel) !== workspace || realpathSync(gitDirectory) === common ||
        inside(workspace, common)) return null;
    return common;
  } catch { return null; }
}
