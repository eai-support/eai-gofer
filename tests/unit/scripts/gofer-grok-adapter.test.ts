...
const RESULT = JSON.stringify({
  text: 'done', stopReason: 'end_turn', total_cost_usd: 0.0123,
  usage: { input_tokens: 10, cache_read_input_tokens: 90, cache_creation_input_tokens: 5, output_tokens: 20, reasoning_tokens: 3, total_tokens: 125 },
});
...
async function start(f: Awaited<ReturnType<typeof repository>>, command: string, extra: Record<string, unknown> = {}) {
  return startLocalGrokInvocation({
    isolatedWorkspace: f.worktree, prompt: 'do the task', modelId: 'grok-4.5', capabilityReceiptHash: 'cap-1',
    allowedWriteScope: ['src/', 'NOTES.md'], command, expectedHead: f.head, usageReporting: true,
    hostOptions: { maxTurns: 5, temporaryRoots: NO_TEMP }, receiptDirectory: await temp('gofer-grok-receipts-'), ...extra,
  });
}
...
describe.skipIf(process.platform === 'win32')('Grok invocation through the shared launcher', () => {
  it('writes the profile before start, removes it after, builds a bounded command, and prices the usage', async () => {
    ...
    expect(argv).toEqual(expect.arrayContaining(['-p', 'do the task', '--cwd', f.worktree, '--always-approve', '--max-turns', '5', '--no-subagents']));
  ...
  it('kills the process at once when the sandbox could not be applied, and rejects the run', async () => {
    const f = await repository();
    const grok = await fakeGrok(`echo 'warning: sandbox could not be applied: profile not found' >&2\nsleep 3\ntouch '${f.worktree}/ran-unsandboxed.txt'\n${OK}`);
    ...
    const invocation = await start(f, grok.script, {
      evidenceDirectory: path.join(evidence, 'e'), objectiveRevision: 'rev', leaseId: 'lease', worktreeReceipt: 'wt',
    });
  ...
  it('fails a write outside the allowed scope and a commit', async () => {
    ...
    const commit = await fakeGrok(`mkdir -p src; printf 'x' > src/f.txt; git -c user.name=x -c user.email=x@x add -A; git -c user.name=x -c user.email=x@x commit -q -m sneaky\n${OK}`);
    await expect((await start(f, commit.script)).wait()).rejects.toThrow('NATIVE_UNAUTHORIZED_GIT_CHANGE');
  });
  ...
  it('fails when Grok stops for any reason other than finishing, or returns something unreadable', async () => {
    ...
    await expect((await start(f, limited.script)).wait()).rejects.toThrow('NATIVE_HOST_REPORTED_ERROR');
  ...
  it('rejects an unpinned command, missing or excessive turn limits, and a layout it cannot protect', async () => {
    ...
    await expect(start(f, grok.script, { hostOptions: { temporaryRoots: NO_TEMP } })).rejects.toThrow('INVALID_NATIVE_REQUEST');
    await expect(start(f, grok.script, { hostOptions: { maxTurns: 500, temporaryRoots: NO_TEMP } })).rejects.toThrow('INVALID_NATIVE_REQUEST');
    await expect(start(f, grok.script, { hostOptions: { maxTurns: 5 } })).rejects.toThrow('LOCAL_SANDBOX_REQUIRED');
    await expect(start({ ...f, worktree: f.base }, grok.script, { expectedHead: undefined })).rejects.toThrow('LOCAL_SANDBOX_REQUIRED');
    expect(existsSync(path.join(f.worktree, '.grok'))).toBe(false);
  });
...
describe('Grok usage', () => {
  it('maps Grok token counts and rejects incomplete usage', () => {
    expect(extractGrokUsage(RESULT)).toEqual({ inputTokens: 105, cachedInputTokens: 90, outputTokens: 20, reportedCostUsd: 0.0123 });
...
describe.skipIf(process.platform === 'win32')('Grok boundary probe', () => {
  ...
  const modelThat = (leak: number[] = [], report = REFUSED) => (_exe: string, args: string[]) => {
    const prompt = args[1];
    const paths = [...prompt.matchAll(/touch (\/\S+)/g)].map((match) => match[1].replace(/\.$/, ''));
    touch(paths[0]);
    for (const index of leak) touch(paths[index]);
    return { status: 0, stdout: JSON.stringify({ text: report, stopReason: 'end_turn' }), stderr: '' };
  };
  const probe = (f: Awaited<ReturnType<typeof repository>>, run: unknown) =>
    probeGrokSandboxBoundary({ workspaceRoot: f.worktree, executable: '/x/grok', temporaryRoots: NO_TEMP, run: run as never });
  ...
  it.each([[1, 'sibling'], [2, 'Git store'], [3, 'temp folder'], [4, 'Grok home']])('fails when write %i (%s) leaks', async (index) => {
    const f = await repository();
    expect(probe(f, modelThat())).toBe('/x/grok');
    expect(probe(f, modelThat([index]))).toBeNull();
    for (const leaked of [path.join(os.tmpdir(), `.gofer-grok-probe-${process.pid}`), path.join(os.homedir(), '.grok', `.gofer-grok-probe-${process.pid}`)]) {
  ...
  it('fails when the model did not report the refusals, when the sandbox failed, and for a bad layout', async () => {
    ...
    const failed = () => ({ status: 0, stdout: JSON.stringify({ text: REFUSED, stopReason: 'end_turn' }), stderr: 'warning: sandbox could not be applied' });
    expect(probe(f, failed)).toBeNull();
    expect(probeGrokSandboxBoundary({ workspaceRoot: f.worktree, executable: 'grok' })).toBeNull();
    expect(probeGrokSandboxBoundary({ workspaceRoot: f.base, executable: '/x/grok', temporaryRoots: NO_TEMP })).toBeNull();
    expect(probeGrokSandboxBoundary({ workspaceRoot: f.worktree, executable: '/x/grok' })).toBeNull();
  });
});
