...
async function start(
  ...
  return startLocalClaudeInvocation({
    ...
    expectedHead: f.head,
    usageReporting: true,
    hostOptions: { maxBudgetUsd: 0.5 },
    receiptDirectory: await temp('gofer-claude-receipts-'),
    ...extra,
  });
}
...
describe.skipIf(process.platform === 'win32')('Claude sandbox policy', () => {
  ...
  it('can also deny reads of protected folders for the shell and the file tools', async () => {
    ...
    const settings = JSON.parse(
      claudeSandboxPolicy(f.worktree, { protectedReadPaths: [secret] })!.settings
    );
...
describe.skipIf(process.platform === 'win32')(
  'Claude invocation through the shared launcher',
  () => {
    it('builds a sandboxed, budget-capped, scope-limited command and prices the usage', async () => {
      ...
      expect(argv[argv.indexOf('--allowedTools') + 1]).toBe(
        'Bash,Read,Glob,Grep,Edit(src/**),Write(src/**),Edit(NOTES.md),Write(NOTES.md)'
      );
    ...
    it('fails a write outside the allowed scope', async () => {
      ...
      await expect((await start(f, claude.script)).wait()).rejects.toThrow(
    ...
    it('removes only an empty Claude bookkeeping folder, and still flags a file placed there', async () => {
      ...
      const hiding = await fakeClaude(
        `mkdir -p .claude/.cc-writes; printf 'x' > .claude/.cc-writes/hidden\n${OK}`
      );
      await expect((await start(f, hiding.script)).wait()).rejects.toThrow(
        'NATIVE_SCOPE_VIOLATION'
      );
    });
    ...
    it('fails when Claude reports an error or returns something unreadable, even with exit code 0', async () => {
      const f = await repository();
      const errored = await fakeClaude(
        `echo '{"type":"result","subtype":"error_max_turns","is_error":true,"result":"x"}'`
      );
      await expect((await start(f, errored.script)).wait()).rejects.toThrow(
    ...
    it('rejects an unpinned command, a missing budget, and a worktree without a separate Git store', async () => {
      ...
      await expect(start(f, claude.script, { hostOptions: {} })).rejects.toThrow(
        'INVALID_NATIVE_REQUEST'
      );
      await expect(start(f, claude.script, { hostOptions: { maxBudgetUsd: 500 } })).rejects.toThrow(
        'INVALID_NATIVE_REQUEST'
      );
      await expect(
        start({ ...f, worktree: f.base }, claude.script, { expectedHead: undefined })
...
describe('Claude usage', () => {
  it('maps Claude token counts and rejects incomplete usage', () => {
    expect(extractClaudeUsage(RESULT)).toEqual({
      inputTokens: 105,
      cachedInputTokens: 90,
      outputTokens: 20,
      reportedCostUsd: 0.0123,
...
describe.skipIf(process.platform === 'win32')('Claude boundary probe', () => {
  ...
  const modelThat =
    (
      behaviour: (paths: { inside: string; outside: string; store: string; tool: string }) => void,
      report = REFUSED
    ) =>
    (_exe: string, args: string[]) => {
      const prompt = args[1];
      const paths = {
        inside: /touch (\S+) \(2\)/.exec(prompt)![1],
        outside: /\(2\) touch (\S+) \(3\)/.exec(prompt)![1],
        store: /\(3\) touch (\S+)\./.exec(prompt)![1],
        tool: /create (\S+) with/.exec(prompt)![1],
      };
      behaviour(paths);
  ...
  it('passes when only the allowed write happens, and cleans up', async () => {
    ...
    const held = probeClaudeSandboxBoundary({
  ...
  it('fails when the model did not report the refusals, even though nothing leaked', async () => {
    const f = await repository();
    const silent = probeClaudeSandboxBoundary({
      workspaceRoot: f.worktree,
      executable: '/x/claude',
  ...
  it('fails when the allowed write did not happen, and for an unpinned command or plain repository', async () => {
    ...
    expect(
      probeClaudeSandboxBoundary({
        workspaceRoot: f.worktree,
        executable: '/x/claude',
        run: modelThat(() => {}) as never,
      })
    ).toBeNull();
    expect(
      probeClaudeSandboxBoundary({ workspaceRoot: f.worktree, executable: 'claude' })
    ).toBeNull();
    expect(
      probeClaudeSandboxBoundary({ workspaceRoot: f.base, executable: '/x/claude' })
    ).toBeNull();
  });
});
