...
const workflow = (name: string) =>
  load(read(`.github/workflows/${name}.yml`)) as {
...
describe('desktop protocol release gate', () => {
  it('runs the full Vitest suite during release validation', () => {
    ...
    const gate = release.slice(
      release.indexOf('run_release_validation_gate() {'),
    ...
    expect(scripts['test:coverage-report']).toContain('vitest run --coverage');
    const qualitySteps = workflow('ci').jobs['quality-gates'].steps!;
    expect(qualitySteps.find((step) => step.name === 'Run Tests with Coverage')?.run).toContain(
  ...
  it('stamps and syncs plugin resources before building the release VSIX', () => {
    const steps = workflow('release').jobs.release.steps!;
    const pluginIndex = steps.findIndex((step) => step.name === 'Package Agent Plugin');
    const syncIndex = steps.findIndex((step) => step.name === 'Sync Extension Resources');
    const buildIndex = steps.findIndex((step) => step.name === 'Build Components');
    const vsixIndex = steps.findIndex((step) => step.name === 'Package VS Code Extension');

    expect(pluginIndex).toBeGreaterThan(-1);
    expect(syncIndex).toBeGreaterThan(pluginIndex);
    expect(buildIndex).toBeGreaterThan(syncIndex);
    expect(vsixIndex).toBeGreaterThan(buildIndex);
  });
  ...
  it('rechecks the committed versioned VSIX before the publish phase creates a tag', () => {
    ...
    expect(phase.indexOf('run_release_validation_gate "$CURRENT_VERSION"')).toBeLessThan(
      phase.indexOf(check)
    );
    expect(phase.indexOf(check)).toBeLessThan(phase.indexOf('git tag "$TAG_NAME"'));
  });

  it.each(['release', 'pages'])(
    'requires exact-SHA desktop checks before %s publication',
    (name) => {
      ...
      expect(publisher.steps?.find((step) => step.uses === 'actions/checkout@v6')?.with?.ref).toBe(
        '${{ github.sha }}'
      );
    }
  );
  ...
  it('preserves standalone CI and pins both reusable jobs to the caller commit', () => {
    ...
    expect(contracts.on.workflow_call.inputs?.['source-sha']).toEqual(
      expect.objectContaining({ required: true, type: 'string' })
    );
    expect(contracts.jobs.protocol.strategy?.matrix.os).toEqual([
      'ubuntu-latest',
      'macos-latest',
      'windows-latest',
    ]);
    for (const name of ['protocol', 'packaged-runtime']) {
      ...
      expect(steps.find((step) => step.uses === 'actions/checkout@v6')?.with?.ref).toBe(
  ...
  it('tests the committed public VSIX, not only a rebuilt package, for every Pages deployment', () => {
    ...
    expect(step?.run).toBe(
      'npm run test:packaged-protocol -- --vsix docs-site/static/releases/eai-gofer-latest.vsix'
    );
  });
  ...
  it('includes locked production dependencies and checks the extracted archive before publication', () => {
    ...
    expect(check.run).toContain(
      "'--runtime-root', path.join(root, 'release-assets', 'language-server')"
    );
    expect(steps.indexOf(check)).toBeLessThan(
      steps.findIndex((step) => step.name === 'Publish GitHub Release')
    );
  });
  ...
  for (const pipefail of [false, true]) {
    it.each([
      ...
      (commandStatus, logStatus) => {
        const release = read('release.sh');
        const helper = release.slice(
          release.indexOf('run_logged_release_check() {'),
          release.indexOf('install_release_dependencies() {')
        );
