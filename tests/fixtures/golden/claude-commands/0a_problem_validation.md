...
Read and follow `.specify/commands/1_gofer_research.md` in the same conversation
under the Continuation And Stop Contract, unless the requested scope ends at
problem validation or an outstanding gate requires a pause. Do not ask for a
numbered command. This helper remains optional in the full pipeline.
