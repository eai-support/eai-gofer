...
export function assertGoferReleaseDescriptor(descriptor: Readonly<GoferReleaseDescriptor>): void {
  if (!descriptor || descriptor.contractVersion !== GOFER_RELEASE_CONTRACT_VERSION) {
    throw new Error(`goferRelease.contractVersion must be ${GOFER_RELEASE_CONTRACT_VERSION}.`);
  }
  if (!REPOSITORY_PATTERN.test(descriptor.repository)) {
