...
- User-facing command and skill pickers should expose only `gofer` and
  `eai-gofer`.
...
If the user is starting a first EAI Platform app, use the public `gofer` or
`eai-gofer` entrypoint, then follow the first-run/setup contract in
...
- Run `eai whoami` before Gofer pipeline work and require a valid login plus an
  active tenant.
