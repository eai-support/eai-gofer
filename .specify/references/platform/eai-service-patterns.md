...
| Documents             | `useDocuments().upload(file, context)`, `classify([file], context)`, or `ragIndex(...)` | `eai docs upload`, `eai docs classify`, or `eai docs index` with authorised context                                                                                | Use the queued ResourceAPI document lifecycle below; index only a persisted document where the configured lifecycle supports it.                                                   |
